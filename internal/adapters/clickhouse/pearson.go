...
func (r *Repository) GettingMarketCapData(coin string) map[int]float64 {
	tokens, err := r.GetCryptoTokens()
	if err != nil {
		fmt.Printf("err: %v\n", err)
		return nil
	...
	id := tokens[coin].ID
	indexQuery := fmt.Sprintf(`SELECT CAST(market_cap AS Float64) FROM cryptowallet.prices WHERE crypto_id = '%s' ORDER BY created_at DESC LIMIT 300`, id)
	rows, err := r.conn.Query(context.Background(), indexQuery)
	if err != nil {
		fmt.Printf("err: %v\n", err)
		return nil
...
func (r *Repository) PearsonPriceToVolumeCorrelation(coin string) float64 {
	...
	averagedY := make(map[int]float64, 50)
	for i := 0; i < 50; i++ {
		var sum float64
		for j := 0; j < 6; j++ {
			sum += y[i*6+j]
